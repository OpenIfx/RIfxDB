...
\noindent
Package \pkg{RIfxDB} implements ODBC database connectivity.  It was
...
Ripley.  Version 1.0-1 was released in January 2003, and \pkg{RIfxDB}
...
Two groups of functions are provided in \pkg{RIfxDB}.  The mainly
...
the client (here \pkg{RIfxDB}) to the DBMSs as specified in the DSN.
Such connections are normally used throughout a session, but should be
closed explicitly at the end of the session---however \pkg{RIfxDB} will
...
\pkg{RIfxDB} functions.  If you forget the details, printing it will
...
The simplest and most common use of \pkg{RIfxDB} is to extract data
...
underscore, and \pkg{RIfxDB} does not in general support vendor
...
quoting rules for names \pkg{RIfxDB} itself sends, but sensible
...
are stored as date-times).  The files in the \code{RIfxDB/tests}
...
change rows in an existing table.  Note that \pkg{RIfxDB} only does
...
\pkg{RIfxDB} allows primary keys to be set as part of the
...
\item[Character types] Character types can be classified three ways:
  ...
  Note that currently \pkg{RIfxDB} only uses the current locale for
...
\item[Dates and times] The handling of dates and times is very much
  ...
  All such types are transferred as character strings in \pkg{RIfxDB}.

\item[Binary types]
  These are less common, and unsupported by \pkg{RIfxDB} prior to
...
\pkg{RIfxDB}) when transferring data and are the same for all drivers.
...
The only ODBC C data types currently used by \pkg{RIfxDB} are
...
\pkg{RIfxDB} usually works with tables in the current schema, but unless
...
supported by the \pkg{RIfxDB} functions.
...
client-server system, and the ODBC client (\pkg{RIfxDB}) and the server
...
this is set when \pkg{RIfxDB} is loaded from the setting of the current
...
\pkg{RIfxDB} is simple to install, and binary distributions are
...
something like
\begin{example}
ODBC_INCLUDE=/path/to/libiodbc-srcs/include' R CMD INSTALL RIfxDB
\end{example}
\noindent
...
RIfxDB connection objects are an integer with several attributes: they
...
handle for the environment that \code{RIfxDB} opens when a connection
...
Prior to \pkg{RIfxDB} 1.3-0 the default was to fetch a row at a time,
